...
import {
  ...
  clearSelectedTodo,
} from "../../store/slice/stackTodosSlice.jsx";
...
export default function Modal() {
  ...
  const closeModal = (event, isKeydown = false) => {
    ...
    if (
      (event.target?.type == "textarea" ||
        event.target?.className == "modal-window MuiBox-root css-1kgfgt1") &&
      !isKeydown
    ) {
      return;
    }
  ...
  return (
    ...
    >
      <Box component="div" className="modal-window" sx={sxModalWindow}>
        <Box
          component="textarea"
          value={selectedTodo?.value}
          onChange={(event) =>
            dispatch(appendNewValueForUpdate(event.target.value))
          }
          onKeyDown={(event) => {
